...
list_file = ['SimpleOneraM6_3.00E-01.dat','SimpleOneraM6_6.00E-01.dat','SimpleOneraM6_8.00E-01.dat']
e=0.93
...
for i,file_name in enumerate(list_file):
    ...
    for j,line in enumerate(lines):
        ...
        new_words=words
        if j == 1:
            new_words = ['#','AoA','Cl','Cdp','Cdp_theory','Cdp_elliptic']
        if len(words)> 0:
            if words[0] != '#':
                AoA=eval(words[0])
                Cl=eval(words[7])
                ...
                Cdp_ell = Cl**2/(numpy.pi*AR)
                new_words=[str(AoA),str(Cl),str(Cdp),str(Cdp_theory),str(Cdp_ell)]
